(function(){
  ...
  function levelFromIndicator(ind){
    ...
    if(ind==='minor') return {level:.50,label:'DEGRADED',color:YELLOW};
  ...
  function infoFromSeverity(rank){
    ...
    if(rank>=2) return {level:.50,label:'DEGRADED',color:YELLOW};
    if(rank>=1) return {level:.50,label:'MAINTENANCE',color:YELLOW};
  ...
  async function loadXai(s){
    ...
    else if(bad) info={level:.22,label:'OUTAGE',color:RED,detail:bad+' xAI service(s) marked outage/down.'};
  ...
  async function refreshOne(s){
    ...
    catch(e){ state[s.id].incidents=state[s.id].incidents||[]; setCard(s.id,{level:.08,label:'FEED ERROR',color:YELLOW,detail:String(e.message||e)},[]); }
  }
